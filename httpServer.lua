...
function Res:send(body)
	...
	local function doSend()
		if buf=='' then 
			...
			--print("send:["..string.sub(buf,1,buffer_size).."]")
			self._skt:send(string.sub(buf,1,buffer_size))
			buf=string.sub(buf,buffer_size+1)
		end
	end
	...
	doSend()
	collectgarbage()
	print("Now mem use:",collectgarbage("count"),"kb")
end
...
function Res:sendFile(filename)
	...
	local function doSend()
		...
		if file.seek('set',pos)==nil then
			...
			local buf=file.read(buffer_size)
			if(nil==buf) then
				self:close()
				file.close()
...
httpServer={_srv=nil,_mids={{url='.*',cb=parseHeader},{url='.*',cb=staticFile}}}

function httpServer:use(url,cb)
	table.insert(self._mids,#self._mids,{url=url,cb=cb})
end
